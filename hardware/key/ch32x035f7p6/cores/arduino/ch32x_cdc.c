...
/* EP5 is the HID keyboard IN endpoint. CH32X035's EP5 registers are at
 * USBFS + 0x54 (DMA), +0x64 (TX length), +0x66 (control high). */
#define HID_DMA      (*(volatile uint32_t *)(USBFS_BASE + 0x54u))
#define HID_TX_LEN   (*(volatile uint16_t *)(USBFS_BASE + 0x64u))
#define HID_CTRL     (*(volatile uint16_t *)(USBFS_BASE + 0x66u))
#define HID_EP5_TX_EN 0x01u

/* Boot keyboard: modifiers, reserved, six simultaneous key usages. */
...
static const uint8_t hid_descr[] = {
    0x09,0x21, 0x11,0x01, 0x00, 0x01,0x22,
    sizeof(hid_report_descr),0x00
};
...
static const uint8_t dev_descr[] = {
    ...
    0x0D, 0xFE,  /* PID: 0xFE0D (keyboard + CDC) */
...
static const uint8_t cfg_descr[] = {
    ...
    /* IAD: group interfaces 0 and 1 as CDC */
    ...
    0x09, 0x21, 0x11, 0x01, 0x00, 0x01, 0x22,
    sizeof(hid_report_descr), 0x00,
    /* EP5 IN: interrupt, 8 bytes, 10ms */
    0x07, 0x05, 0x85, 0x03, 0x08, 0x00, 0x0A,
};
...
static const uint8_t prod_descr[] = {
    0x16, 0x03,
    'U',0, 'S',0, 'B',0, ' ',0, 'K',0, 'e',0, 'y',0, 'p',0, 'a',0, 'd',0
};
...
static volatile uint8_t hid_busy;
static volatile uint8_t hid_protocol = 1;
static volatile uint8_t hid_idle;
__attribute__((aligned(4))) static uint8_t hid_tx_dma[8];
...
static void endp_init(void) {
    ...
    USBFSD->UEP567_MOD = HID_EP5_TX_EN;
    HID_DMA = (uint32_t)hid_tx_dma;
...
void USBFS_IRQHandler(void) {
    ...
    if (intflag & USBFS_UIF_TRANSFER) {
        ...
        case USBFS_UIS_TOKEN_SETUP: {
            ...
            if ((setupReqType & USB_REQ_TYP_MASK) == USB_REQ_TYP_CLASS &&
                ...
                case 0x01: /* GET_REPORT */
                    pDescr = empty_report; len = 8; break;
                case 0x02: ep0_buf[0] = hid_idle; setupReqLen = 1; break; /* GET_IDLE */
                case 0x03: ep0_buf[0] = hid_protocol; setupReqLen = 1; break;
                case 0x09: break; /* SET_REPORT: output LEDs */
                case 0x0A: hid_idle = (uint8_t)(setupReqValue >> 8); break;
                case 0x0B: hid_protocol = (uint8_t)setupReqValue; break;
                default: err = 1; break;
                }
            }
            else if ((setupReqType & USB_REQ_TYP_MASK) == USB_REQ_TYP_CLASS &&
                     setupReqIndex == 0) {
                switch (setupReqCode) {
                ...
                case USB_GET_DESCRIPTOR:
                    ...
                    case USB_DESCR_TYP_STRING:
                        ...
                        break;
                    case 0x21: /* HID descriptor, interface 2 only */
                        if (setupReqIndex != 2) { err = 1; break; }
                        pDescr = hid_descr; len = sizeof(hid_descr); break;
                    case 0x22: /* HID report descriptor */
                        if (setupReqIndex != 2) { err = 1; break; }
                        pDescr = hid_report_descr; len = sizeof(hid_report_descr); break;
                    default: err = 1; break;
                    }
                    if (!err) {
                        if (setupReqLen > len) setupReqLen = len;
                ...
                case USB_SET_CONFIGURATION:
                    if (setupReqValue > 1) { err = 1; break; }
                ...
                case USB_CLEAR_FEATURE:
                    if ((setupReqType & USB_REQ_RECIP_MASK) == USB_REQ_RECIP_ENDP) {
                        ...
                        case (UEP_IN|5):  HID_CTRL = USBFS_UEP_T_RES_NAK; break;
                        default: err = 1; break;
                        }
                    } else {
                ...
                case USB_SET_FEATURE:
                    if ((setupReqType & USB_REQ_RECIP_MASK) == USB_REQ_RECIP_ENDP &&
                        ...
                        case (UEP_IN|5):
                            HID_CTRL = (HID_CTRL & ~USBFS_UEP_T_RES_MASK) | USBFS_UEP_T_RES_STALL; break;
                        default: err = 1; break;
                        }
                    } else { err = 1; }
                    break;
                ...
                case USB_GET_STATUS:
                    ...
                    if ((setupReqType & USB_REQ_RECIP_MASK) == USB_REQ_RECIP_ENDP) {
                        ...
                        case (UEP_IN|3):
                            ...
                            break;
                        case (UEP_IN|5):
                            if ((HID_CTRL & USBFS_UEP_T_RES_MASK) == USBFS_UEP_T_RES_STALL) ep0_buf[0] = 1;
                            break;
                        default: err = 1; break;
                        }
                    }
        ...
        case USBFS_UIS_TOKEN_IN:
            if (ep == 5) {
                HID_CTRL ^= USBFS_UEP_T_TOG;
                HID_CTRL = (HID_CTRL & ~USBFS_UEP_T_RES_MASK) | USBFS_UEP_T_RES_NAK;
                hid_busy = 0;
            } else if (ep == 3) {
        ...
        case USBFS_UIS_TOKEN_OUT:
            if (ep == 0) {
                if (intst & USBFS_UIS_TOG_OK) {
                    if ((setupReqType & USB_REQ_TYP_MASK) == USB_REQ_TYP_CLASS) {
                        if (setupReqIndex == 2) {
                            /* HID SET_REPORT: accept LED output byte, no LEDs wired. */
                            setupReqLen = 0;
                        } else if (setupReqIndex == 0) {
...
/* Non-blocking; caller retries the latest state when the USB host is busy. */
int ch32x_hid_send(const uint8_t report[8]) {
    if (!devConfig || hid_busy) return 0;
    memcpy(hid_tx_dma, report, 8);
    HID_TX_LEN = 8;
    hid_busy = 1;
    HID_CTRL = (HID_CTRL & ~USBFS_UEP_T_RES_MASK) | USBFS_UEP_T_RES_ACK;
    return 1;
}
